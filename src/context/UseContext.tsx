...
export interface AuthContextType {
  ...
  setWorkshopLocation: React.Dispatch<
...
const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [phoneNumber, setPhoneNumber] = useState(() =>
    getStoredValue("auth_phoneNumber", ""),
  );
  const [code, setCode] = useState(() => getStoredValue("auth_code", ""));
  const [selectRole, setSelectRole] = useState(() =>
    getStoredValue("auth_selectRole", ""),
  );
  ...
  const [resetToken, setResetToken] = useState(() =>
    getStoredValue("auth_resetToken", ""),
  );
  // Initial state olishda xatolikdan qochish
  const [probeg, setProbeg] = useState<number>(() => {
    try {
      const saved = localStorage.getItem("probeg");
      // "null", "undefined" yoki bo'sh bo'lsa 0 qaytaradi
  ...
  // Faqat probeg 0 dan katta bo'lgandagina saqlash (ixtiyoriy, lekin tavsiya etiladi)
  useEffect(() => {
    // Agar probeg 0 bo'lsa ham saqlashni istasangiz, shunchaki probeg !== undefined qoldiring
    if (probeg !== undefined && probeg !== null) {
      localStorage.setItem("probeg", JSON.stringify(probeg));
    }
  }, [probeg]);
  ...
  const [workshopTitle, setWorkshopTitle] = useState(() =>
    getStoredValue("workshop_title", ""),
  );
  const [workshopAddress, setWorkshopAddress] = useState(() =>
    getStoredValue("workshop_address", ""),
  );
  const [workshopDescription, setWorkshopDescription] = useState(() =>
    getStoredValue("workshop_description", ""),
  );
  const [workshopWorkingTime, setWorkshopWorkingTime] = useState(() =>
    getStoredValue("workshop_workingTime", ""),
  );
  ...
  const [workshopPasswordConfirm, setWorkshopPasswordConfirm] =
    useState<string>("");
  ...
  useEffect(() => {
    sessionStorage.setItem(
      "auth_isForgotPassword",
      JSON.stringify(isForgotPassword),
    );
  }, [isForgotPassword]);
  useEffect(() => {
  ...
  useEffect(() => {
    sessionStorage.setItem(
      ...
      JSON.stringify(workshopDescription),
    );
  }, [workshopDescription]);
  useEffect(() => {
    sessionStorage.setItem(
      "workshop_workingTime",
      JSON.stringify(workshopWorkingTime),
    );
  }, [workshopWorkingTime]);
  useEffect(() => {
    sessionStorage.setItem(
      "workshop_location",
      JSON.stringify(workshopLocation),
    );
  }, [workshopLocation]);
  useEffect(() => {
